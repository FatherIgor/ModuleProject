...
public class Task {
    ...
    public Task(String title, int time) throws IllegalArgumentException {
        if (title == null || title.isEmpty() || time < 0)
    ...
    public Task(String title, int start, int end, int repeat) throws IllegalArgumentException {
        if (title == null || title.isEmpty() || start < 0 || end < 0 || repeat < 0 || start > end || repeat > end - start)
            throw new IllegalArgumentException();
        //checkArgument(!title.isEmpty(), "Title must not be empty");
        this.title = title;
        this.start = start;
    ...
    public void setTime(int start, int end, int repeat) throws IllegalArgumentException {
        if (start < 0 || end < 0 || repeat < 0 || start > end || repeat > end - start)
            throw new IllegalArgumentException();
        this.start = start;
        this.repeat = repeat;
        this.end = end;
